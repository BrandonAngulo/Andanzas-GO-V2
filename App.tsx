...
export default function App() {
  ...
  // Fake user for testing when auth is bypassed
  const effectiveUser = user || (isAuthenticated ? { id: 'test-user', email: 'test@example.com' } : null);
  ...
  useEffect(() => {
    if (isAuthenticated && effectiveUser) {
      if (effectiveUser.id !== 'test-user') {
        userService.getFavorites(effectiveUser.id).then(setFavIds);
        reviewsService.getByUserId(effectiveUser.id).then(setReviews);
        routesService.getUserRoutes(effectiveUser.id).then(setRutas);
        notificationsService.getUserNotifications(effectiveUser.id).then(setNotifications);
        gamificationService.getBadgesForUser(effectiveUser.id).then(badges => {
          setAllInsignias(badges);
          setEarnedInsignias(badges.filter(b => b.obtenida).map(b => b.id));
        });
        // Check for onboarding
        userService.getProfile(effectiveUser.id).then(profile => {
  ...
  const toggleFav = async (id: string) => {
    ...
    if (isAuthenticated) { // Changed for effectiveUser support
      try {
        if (effectiveUser && effectiveUser.id !== 'test-user') {
          if (isCurrentlyFav) {
            await userService.removeFavorite(effectiveUser.id, id);
          } else {
            await userService.addFavorite(effectiveUser.id, id);
          }
        }
      } catch (error) {
  ...
  const addReview = async (siteId: string, text: string, rating: number, fotos: File[]) => {
    ...
    if (isAuthenticated && effectiveUser && effectiveUser.id !== 'test-user') {
      reviewsService.addReview({ siteId, text, rating, fotos }, effectiveUser.id).catch(console.error);
    }
  };
  ...
  const saveNewRoute = async (name: string, description: string) => {
    if (!isAuthenticated || !name || newRoutePoints.length === 0 || !effectiveUser) return;
    const ruta: Ruta = {
      id: `r_${Date.now()}`,
      nombre: name,
    ...
    try {
      if (effectiveUser.id !== 'test-user') {
        const savedRoute = await routesService.createRoute(ruta, effectiveUser.id);
        if (savedRoute) {
