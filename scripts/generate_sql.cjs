...
for (const entry of entries) {
  const t = entry.title.replace(/'/g, "''");
  const c = entry.city.replace(/'/g, "''");
  const tg = toPgArray(entry.tags);
  const cf = entry.content_full.replace(/'/g, "''");
  const sq = toPgArray(entry.sabias_que);
  const tr = entry.trivia.replace(/'/g, "''");
  const cta = entry.cta.replace(/'/g, "''");
  const f = entry.fuentes.replace(/'/g, "''");
  const sid = toPgArray(entry.site_ids);
  const rid = toPgArray(entry.route_ids);
  
  sql += `INSERT INTO learn_entries (title, city, tags, content_full, sabias_que, trivia, cta, fuentes, site_ids, route_ids) VALUES ('${t}', '${c}', ${tg}, '${cf}', ${sq}, '${tr}'::jsonb, '${cta}', '${f}', ${sid}, ${rid});\n`;
}

fs.writeFileSync('scripts/insert_block2.sql', sql);
