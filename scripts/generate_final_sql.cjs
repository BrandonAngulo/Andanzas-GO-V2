...
// THE BASE LIST (User Manual Input)
const corrections = [
...
// Logic: START with User's Manual Coords (from corrections array) as the DEFAULT.
// THEN, iterate matches. If match is "GOOD", OVERWRITE.
// RESULT: Skipped items stay as User Manual. Matched items become Google Coords.

let sql = '';

for (const site of corrections) {
    let finalLat = site.lat;
    let finalLng = site.lng;
    let source = "MANUAL";

    const match = matches.find(m => m.id === site.id);
    if (match) {
        // Evaluate Match Quality
        const dbName = normalize(site.nombre);
        ...
        const isGoodMatch = sim > 0.4 || contains;

        if (isGoodMatch) {
            finalLat = match.new_lat;
            finalLng = match.new_lng;
            source = "GOOGLE";
        }
    }

    // Construct SQL Update
    sql += `UPDATE sites SET lat = ${finalLat}, lng = ${finalLng} WHERE id = '${site.id}';\n`;
}

fs.writeFileSync(path.join(__dirname, 'final_updates.sql'), sql, 'utf8');
