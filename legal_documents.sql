-- =========================================================================
-- Script unico para configurar los DOCUMENTOS LEGALES en Supabase.
-- Copialo y pegalo en el editor SQL de Supabase y ejecutalo.
--
-- Equivale a aplicar estas migraciones (misma fuente, sin divergencia):
--   supabase/migrations/20260709_legal_documents.sql  (esquema + RLS)
--   supabase/migrations/20260714_seed_legal_documents.sql  (contenido)
--
-- Es idempotente: puede ejecutarse varias veces sin duplicar ni romper.
-- =========================================================================
...
DROP POLICY IF EXISTS "Public Read Published Legal Docs" ON public.legal_documents;
CREATE POLICY "Public Read Published Legal Docs" ON public.legal_documents 
    FOR SELECT USING (status = 'published');

DROP POLICY IF EXISTS "Admin All Access Legal Docs" ON public.legal_documents;
...
DROP POLICY IF EXISTS "Users can read own acceptances" ON public.user_legal_acceptances;
CREATE POLICY "Users can read own acceptances" ON public.user_legal_acceptances 
    FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert own acceptances" ON public.user_legal_acceptances;
CREATE POLICY "Users can insert own acceptances" ON public.user_legal_acceptances 
    FOR INSERT WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Admin read all acceptances" ON public.user_legal_acceptances;
...
CREATE POLICY "Users can view own appeals" ON public.moderation_appeals 
    FOR SELECT USING (auth.uid() = user_id);
DROP POLICY IF EXISTS "Users can create appeals" ON public.moderation_appeals;
CREATE POLICY "Users can create appeals" ON public.moderation_appeals 
    FOR INSERT WITH CHECK (auth.uid() = user_id);
DROP POLICY IF EXISTS "Admins can manage appeals" ON public.moderation_appeals;
