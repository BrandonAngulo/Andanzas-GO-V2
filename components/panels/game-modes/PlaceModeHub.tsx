...
export const PlaceModeHub: React.FC<Props> = ({ game, themes, onClose, onPlay }) => {
    const places = useMemo(() => themes
        .filter(theme => theme.kind === 'place')
        .sort((a, b) => (a.sortOrder ?? 999) - (b.sortOrder ?? 999) || a.label.localeCompare(b.label, 'es')), [themes]);
    ...
    return <ModeLobbyShell eyebrow="TRIVIA GO · Antesala de modo" title="Jugar por lugar" subtitle="Explora el mundo eligiendo un territorio concreto." onClose={onClose}>
        <section className="relative overflow-hidden rounded-[1.5rem] bg-gradient-to-br from-[#06434a] via-[#08705d] to-[#10a36f] px-4 py-4 text-white shadow-xl sm:px-5 sm:py-4">
            ...
            <div className="relative z-10 grid items-center gap-2 sm:grid-cols-[1fr_10rem]">
                <div>
                    <span className="inline-flex items-center gap-1 rounded-full bg-amber-300 px-2.5 py-1 text-[8px] font-black uppercase tracking-[0.16em] text-amber-950"><Globe2 className="h-3 w-3" /> Mundo · selector de territorios</span>
                    <h2 className="mt-2 text-2xl font-black sm:text-3xl">¿A dónde quieres viajar?</h2>
                    <p className="mt-1 max-w-2xl text-xs leading-relaxed text-white/75 sm:text-sm">El mundo se abre por destinos. Elige un país, una región o una ciudad y juega con preguntas propias de ese lugar.</p>
                </div>
        ...
        {places.length === 0 ? (
            ...
            <div className="mt-3 grid gap-3 lg:grid-cols-[minmax(0,1.35fr)_minmax(20rem,0.65fr)]">
                <section className="rounded-2xl border border-emerald-900/10 bg-white/90 p-3 shadow-sm">
                    <div className="mb-2 flex items-center justify-between gap-2"><div><p className="text-[8px] font-black uppercase tracking-[0.16em] text-emerald-700">Destinos del mundo</p><h3 className="text-sm font-black">Elige tu territorio</h3></div><span className="rounded-full bg-emerald-50 px-2.5 py-1 text-[8px] font-bold text-emerald-700">{places.length} disponibles</span></div>
                    <div className="grid gap-2 sm:grid-cols-2">{places.map(theme => {
                        ...
                        >
                            ...
                            <span className="min-w-0 flex-1"><span className="block text-[7px] font-black uppercase tracking-[0.14em] text-slate-500">{item.scope}</span><strong className="block truncate text-sm font-black text-[#073c43]">{theme.label}</strong><span className="block text-[9px] text-slate-500">{theme.questionCount || 0} preguntas</span></span>
                ...
                {selected && detail ? <aside className="overflow-hidden rounded-2xl border border-emerald-900/10 bg-white shadow-lg">
                    ...
                    <div className="space-y-2.5 p-3">
                        <div className="grid grid-cols-2 gap-2"><MiniStat value={String(game.questions_per_match || 15)} label="por ronda" /><MiniStat value={`${selected.questionCount || 0}`} label="disponibles" /><MiniStat value={loadingSummary ? '…' : String(summary?.completed || 0)} label="completadas" /><MiniStat value={loadingSummary ? '…' : (summary?.best_score || 0).toLocaleString('es-CO')} label="mejor puntaje" /></div>
