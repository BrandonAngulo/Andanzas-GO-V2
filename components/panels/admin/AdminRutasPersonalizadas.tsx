...
import { Map, Mail, Users, Clock, Save, FileText, Search, Phone, Building2, Accessibility, CalendarClock, RefreshCcw, Trash2 } from 'lucide-react';
...
export const AdminRutasPersonalizadas: React.FC = () => {
    ...
    const [toDelete,setToDelete] = useState<RequestWithProfile|null>(null);
    ...
    useEffect(() => { load(); }, []);
    const confirmDelete = async () => { if(!toDelete)return; setDeleting(true); try{await customRoutesService.remove(toDelete.id); toast.success('Solicitud eliminada'); setToDelete(null); await load();}catch{toast.error('No se pudo eliminar la solicitud. Revisa tus permisos.');}finally{setDeleting(false);} };
    ...
    return <div className="space-y-6 pb-24 min-h-[85vh]">
        ...
        {loading?<div className="py-12 text-center animate-pulse">Cargando solicitudes…</div>:visible.length===0?<div className="py-12 text-center border-2 border-dashed rounded-2xl text-muted-foreground">No hay solicitudes con estos filtros.</div>:<div className="grid xl:grid-cols-2 gap-5">{visible.map(req=><Card key={req.id} className="overflow-hidden"><CardContent className="p-0">
            <div className="p-4 border-b bg-muted/30 flex justify-between gap-3"><div><h4 className="font-bold">{req.contact_name||req.profile?.full_name||'Sin nombre'}</h4><div className="flex flex-wrap gap-x-3 text-xs text-muted-foreground mt-1"><span className="flex gap-1"><Mail className="w-3 h-3"/>{req.contact_email||req.profile?.email}</span><span className="flex gap-1"><Phone className="w-3 h-3"/>{req.contact_phone||'Sin teléfono'}</span></div></div><div className="flex flex-col items-end gap-1 shrink-0"><span className={`inline-block px-2 py-1 rounded-full text-[11px] font-bold ${statusTone[req.status]}`}>{STATUS[req.status]}</span><p className="text-[10px] text-muted-foreground">{new Date(req.created_at).toLocaleDateString('es-CO')}</p><Button size="sm" variant="ghost" className="h-7 gap-1 px-2 text-destructive hover:bg-destructive/10 hover:text-destructive" onClick={()=>setToDelete(req)}><Trash2 className="w-3.5 h-3.5"/><span className="text-xs">Eliminar</span></Button></div></div>
        ...
        <ConfirmDialog
            ...
            description={toDelete ? `Se eliminará de forma permanente la solicitud de ${toDelete.contact_name||toDelete.profile?.full_name||'este usuario'}. Esta acción no se puede deshacer.` : undefined}
