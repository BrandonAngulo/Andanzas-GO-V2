    ...
    return <Dialog open={open} onOpenChange={close}><DialogContent className="sm:max-w-2xl max-h-[92vh] overflow-y-auto">
        ...
        <form onSubmit={submit} className="space-y-6 mt-4">
            ...
            <div className="space-y-3 rounded-2xl border p-4">
                <div className="flex flex-wrap items-center justify-between gap-2"><strong className="text-sm flex items-center gap-1"><ShieldCheck className="w-4 h-4" />Reglas y políticas de la experiencia</strong><Button type="button" size="sm" variant="outline" onClick={() => setShowRules(previous => !previous)}>{showRules ? 'Ocultar decálogo' : 'Consultar decálogo básico'}</Button></div>
                {showRules && <ol className="grid list-decimal gap-x-6 gap-y-1 pl-5 text-xs text-muted-foreground sm:grid-cols-2"><li>Seguir las indicaciones del guía.</li><li>Llegar a la hora acordada.</li><li>Respetar comunidad y patrimonio.</li><li>Cumplir las normas de seguridad.</li><li>Informar condiciones relevantes.</li><li>Supervisar siempre a los menores.</li><li>Cuidar el entorno y no dejar residuos.</li><li>Solicitar permiso antes de fotografiar personas.</li><li>Mantener un trato respetuoso.</li><li>Aceptar ajustes por clima, seguridad o fuerza mayor.</li></ol>}
                <p className="text-xs text-muted-foreground">Consulta también nuestros <a href="/terms" target="_blank" rel="noopener noreferrer" className="font-medium text-primary underline">Términos de Servicio</a> y la <a href="/privacy" target="_blank" rel="noopener noreferrer" className="font-medium text-primary underline">Política de Privacidad</a>.</p>
                <label className="flex cursor-pointer items-start gap-3"><input type="checkbox" className="mt-1" checked={form.rules_accepted} onChange={e => set('rules_accepted',e.target.checked)} /><span><strong className="text-sm">Acepto las reglas básicas de la experiencia</strong><span className="mt-1 block text-xs text-muted-foreground">Confirmo que el grupo conoce estas condiciones y seguirá las instrucciones operativas de Andanzas.</span></span></label>
            </div>
