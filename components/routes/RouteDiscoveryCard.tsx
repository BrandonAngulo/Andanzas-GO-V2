...
export function RouteDiscoveryCard({
  ...
  return (
    <article
      className={cn(
        'group relative isolate overflow-hidden rounded-[1.75rem] border border-emerald-950/10 bg-white shadow-[0_18px_55px_-35px_rgba(6,78,59,0.75)] transition-transform duration-300 dark:border-white/10 dark:bg-slate-950',
        featured ? 'min-h-[25rem] sm:min-h-[27rem]' : 'min-h-[23rem]',
    ...
    >
      ...
      <div className="absolute inset-0">
        ...
        />
        <div className="absolute inset-0 bg-gradient-to-b from-black/5 via-black/10 to-[#052f2b]/95" />
        <div className="absolute inset-x-0 bottom-0 h-2/3 bg-[radial-gradient(circle_at_20%_0%,rgba(16,185,129,0.24),transparent_45%)]" />
      </div>

      <div className="relative flex min-h-[inherit] flex-col p-4 sm:p-5">
        <div className="flex items-start justify-between gap-3">
          <span className={cn('inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-[11px] font-black uppercase tracking-[0.12em] shadow-sm', status.tone)}>
        ...
        <div className="mt-auto text-white">
            ...
            <div className="mb-4 flex items-center">
              {routeSites.slice(0, 5).map((site, index) => (
                <React.Fragment key={site.id}>
                  <span className={cn(
                    'grid h-7 w-7 shrink-0 place-items-center rounded-full border-2 border-white/80 text-[10px] font-black shadow-md',
                    index === 0 ? 'bg-orange-400 text-orange-950' : 'bg-emerald-600 text-white',
                  )}>
                    {index + 1}
                  </span>
                  {index < Math.min(routeSites.length, 5) - 1 && (
                    <span className="h-px w-5 border-t border-dashed border-white/70 sm:w-7" />
                  )}
                </React.Fragment>
              ))}
              {routeSites.length > 5 && (
          ...
          <h3 className={cn('font-heading font-black leading-[1.03] text-white', featured ? 'text-3xl sm:text-4xl' : 'text-2xl')}>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2 text-xs font-semibold">
            ...
            <span className="ml-auto inline-flex items-center gap-1.5 rounded-full bg-white px-3 py-1.5 font-black text-emerald-950">
