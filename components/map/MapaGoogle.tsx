...
import { Layers, Loader2, Navigation, MapPin, AlertTriangle, X, Info, Star, Filter } from 'lucide-react';
...
const CustomPin = ({ color, icon, number }: { color: string, icon: string, number?: number }) => (
    <div className="relative group cursor-pointer transform transition-all duration-300 hover:scale-110 hover:-translate-y-1 origin-bottom">
        ...
        <svg width="34" height="42" viewBox="0 0 34 42" fill="none" xmlns="http://www.w3.org/2000/svg" className="drop-shadow-sm opacity-50 group-hover:opacity-100 transition-opacity">
                ...
                <div className="absolute top-3 right-3 flex flex-col items-end gap-2 pointer-events-auto">
                    ...
                    <div className="relative">
                        ...
                        >
                            ...
                            <span className="hidden md:inline font-semibold">{language === 'es' ? 'Filtros' : 'Filters'}</span>
                            {props.selectedCategories.length > 0 && (
                                <span className="absolute -top-1.5 -right-1.5 flex h-5 w-5 items-center justify-center rounded-full bg-primary text-[10px] font-bold text-primary-foreground md:static md:ml-1.5 ring-2 ring-white">
