...
1. ✔ **Hecho.** Taxonomía de eventos mínima conectada a `analyticsService.trackEvent` en los puntos de disparo del motor (ver §5). Aditivo, sin cambiar reglas de juego.
2. ✔ **Hecho.** Dashboard base (embudo por modo y exactitud por dificultad) en `JuegosAnalyticsPanel`, desde `analytics_events` + `game_answers`. *Nota:* D1/D7 (retorno por cohorte) se difiere a un RPC de reporte pequeño — con ~24 sesiones el cálculo en cliente no es representativo todavía.
...
- **Métricas básicas disponibles:** ✔ instrumentación desplegada — **pendiente de datos reales** (los eventos se poblarán con el uso tras el redeploy).

**Ejecutado:**
- **P0.1** ✔ instrumentación mínima de eventos desplegada (`useGameEngine`, `challenge.service`, `JuegosPanel`). Verificado contra V3: la política RLS `INSERT` (user_id=auth.uid(), tamaños, metadata jsonb ≤4096) admite los eventos; la política `SELECT "Admins can view analytics"` permite leerlos en el panel.
- **P0.2** ✔ dashboard base en `JuegosAnalyticsPanel` (embudo, partidas por modo, exactitud por dificultad).

**Siguiente:** con la señal poblándose, la **Fase 1 (estabilización del duelo)** ya puede diseñarse/implementarse — su causa raíz está diagnosticada (R1) y requiere migración (versión de ruleset, set congelado, resolución en servidor), por lo que se decidirá rama-vs-producción al arrancarla.
